...
export default async function MarkdownPreview({ description, className = '' }: MarkdownPreviewProps) {
  ...
  const components: Components = {
    ...
    h5({ children }) {
      return <h5 className="text-base font-semibold text-gray-900 dark:text-white mt-3 mb-1">{children}</h5>;
    },
    h6({ children }) {
      return <h6 className="text-sm font-semibold text-gray-900 dark:text-white mt-2 mb-1 uppercase tracking-wide">{children}</h6>;
    },
...
li({ children }) {
  return (
    <li className="list-disc list-outside ml-5 text-gray-700 dark:text-gray-300 leading-relaxed marker:text-gray-700 dark:marker:text-gray-400">
      {children}
    </li>
  );
},
